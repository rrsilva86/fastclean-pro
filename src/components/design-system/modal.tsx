...
export function Modal({ children, onClose, size = "default", title }: { children: ReactNode; onClose: () => void; size?: ModalSize; title: string }) {
  const widthClass = size === "wide" ? "max-w-[96vw]" : "max-w-3xl";

  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/35 p-3 backdrop-blur-sm" onClick={onClose}>
      <section className={`max-h-[92vh] w-full ${widthClass} overflow-hidden rounded-xl border border-slate-200 bg-white shadow-premium`} onClick={(event) => event.stopPropagation()}>

...
const settingsNavigationItem = { key: "nav.settings", href: "settings", icon: Settings2, module: "settings", permission: "settings.manage" };
...
export function AppSidebar({ locale, session }: { locale: Locale; session: AppSession }) {
  ...
  return (
    <>
      <aside className="group/sidebar sticky top-0 hidden h-screen w-[68px] shrink-0 overflow-hidden border-r border-slate-200 bg-white px-2 py-6 transition-[width] duration-300 ease-out hover:w-[288px] lg:block">
        ...
        <nav className="mt-4 grid w-[272px] gap-2">
          ...
          {(() => {
            ...
            const isActive = pathname === `/${locale}/${settingsNavigationItem.href}`;
            const access = canAccessFeature({
              tenant: session.tenant,
              role: session.role,
              module: settingsNavigationItem.module as AppModule,
            ...
            return (
              <div className="mt-4 border-t border-slate-100 pt-4">
                <Link
                  ...
                  href={`/${locale}/${settingsNavigationItem.href}`}
                >
                  <Icon className="h-5 w-5 shrink-0" strokeWidth={1.9} />
                  <span className="truncate opacity-0 transition-opacity duration-200 group-hover/sidebar:opacity-100">{t(settingsNavigationItem.key)}</span>
                </Link>
              </div>
            );
          })()}
      ...
      <nav className="fixed inset-x-0 bottom-0 z-50 grid grid-cols-5 gap-1 border-t border-slate-200 bg-white/95 px-2 pb-[calc(0.5rem+env(safe-area-inset-bottom))] pt-2 shadow-[0_-12px_30px_rgba(15,23,42,0.10)] backdrop-blur lg:hidden" aria-label={t("nav.menu")}>
        {mobileNavigationItems.map((item) => {
          ...
          return (
            <Link
              aria-disabled={!access.allowed}
              className={`flex min-w-0 flex-col items-center justify-center gap-1 rounded-2xl px-1 py-2 text-[10px] font-extrabold transition-all duration-200 ${
                !access.allowed
                  ? "pointer-events-none text-slate-300"
                  : isActive
                    ? "bg-cyan-50 text-cyan-700 ring-1 ring-cyan-100"
                    : "text-slate-500 active:scale-[0.98] active:bg-slate-50"
              }`}
              href={`/${locale}/${item.href}`}
              key={item.key}
            >
